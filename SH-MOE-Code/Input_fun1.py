...
def apply_gate_transforms(df: pd.DataFrame,
    ...
    def q_to_quantile(q):
        # 给定流量 q，把它投到 FDC 上得到分位（0-1）
        # np.interp: x -> y, 这里用 q_grid(升序) 对应 p_grid(0..1)
        # 注意边界：小于最小流量给 0，大于最大流量给 1
        return float(np.interp(q, q_grid, p_grid, left=0.0, right=1.0))

    # 统一用分站点 + _to_rank
    def _to_rank(q):
        if pd.isna(q): return np.nan
        r = float(np.interp(q, q_grid, p_grid, left=0.0, right=1.0))
        return max(0.0, min(1.0, r))

    q_t1 = df.groupby("section_id")["measured_flow"].shift(1)
    q_t2 = df.groupby("section_id")["measured_flow"].shift(2)

    df["gate_r_t1"] = q_t1.apply(_to_rank)
    df["gate_r_t2"] = q_t2.apply(_to_rank)
    df["gate_dr_t1"] = df["gate_r_t1"] - df["gate_r_t2"]
